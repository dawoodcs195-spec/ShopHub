...
const DailySiteStat = require("../models/DailySiteStat"); // ✅ NEW

const getUTCDateKey = () => new Date().toISOString().slice(0, 10); // YYYY-MM-DD (UTC)
...
const getDashboardStats = async (req, res) => {
  try {
    ...
    const monthlyRevenue = monthlyRevenueAggregation.map((item) => ({
    ...
    const paymentMethods = paymentMethodsAggregation.map((item) => ({
      name: item._id,
      value: item.value,
    }));

    // ===============================
    // ✅ Visitor Analytics (today + totals)
    // ===============================
    const todayKey = getUTCDateKey();

    const todayStat = await DailySiteStat.findOne({ dateKey: todayKey }).lean();

    const totalsAgg = await DailySiteStat.aggregate([
      {
        $group: {
          _id: null,
    ...
    const totals = totalsAgg?.[0] || { totalVisits: 0, totalUniqueVisitors: 0 };
    ...
    return res.status(200).json({
      ...
      dashboard: {
        ...
        // ✅ Visitors
        todayTotalVisits: todayStat?.totalVisits || 0,
        todayUniqueVisitors: todayStat?.uniqueVisitors || 0,
        totalVisits: totals.totalVisits || 0,
        totalUniqueVisitors: totals.totalUniqueVisitors || 0,
