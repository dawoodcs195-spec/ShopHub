    ...
    <div className="overflow-x-auto bg-surface rounded-lg shadow-soft animate-pulse">
        <table className="min-w-full">
            <thead className="bg-background">
                <tr>
                    {Array.from({ length: 7 }).map((_, i) => <th key={i} className="px-4 py-3"><div className="h-4 bg-gray-200 rounded"></div></th>)}
                </tr>
            </thead>
            <tbody>
                {Array.from({ length: 10 }).map((_, i) => (
                    <tr key={i} className="border-b border-border">
                        {Array.from({ length: 7 }).map((_, j) => <td key={j} className="px-4 py-3"><div className="h-5 bg-gray-200 rounded"></div></td>)}
                    </tr>
                ))}
            </tbody>
        </table>
...
const AdminOrders = () => {
    ...
    const limit = 10;
    const [filters, setFilters] = useState({ search: "", orderStatus: "", paymentStatus: "", paymentMethod: "", startDate: "", endDate: "" });
    ...
    const handleClearFilters = useCallback(() => {
        setFilters({ search: "", orderStatus: "", paymentStatus: "", paymentMethod: "", startDate: "", endDate: "" });
        setPage(1);
    }, []);
    ...
    const handleStatusChange = useCallback(async (id, status) => {
        try {
            setOrders(prev => prev.map(order => order._id === id ? { ...order, orderStatus: status } : order));
